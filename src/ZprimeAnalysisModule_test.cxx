...
class ZprimeAnalysisModule_test : public ModuleBASE {

public:
  explicit ZprimeAnalysisModule_test(uhh2::Context&);
...
void ZprimeAnalysisModule_test::book_histograms(uhh2::Context& ctx, vector<string> tags){
...
void ZprimeAnalysisModule_test::fill_histograms(uhh2::Event& event, string tag){
  string mytag = tag + "_Skimming";
  mytag = tag+"_General";
...
ZprimeAnalysisModule_test::ZprimeAnalysisModule_test(uhh2::Context& ctx){
  //debug = true;
  debug = false;
...
//  BTagWeight_module.reset(new MCBTagDiscriminantReweighting(ctx, btag_algo, "jets", Sys_btag));
  ...
  if(is2017v2 && isMuon){
    ...
    HT_module.reset(new MCNjetsHTScaleFactor(ctx, "/nfs/dust/cms/user/hugobg/UHH2_v2/CMSSW_10_2_17/src/UHH2/common/data/2017/HT_SF.root", 1.0, "HT", Sys_EleID));
  }
  if(is2018 && isMuon){
...
bool ZprimeAnalysisModule_test::process(uhh2::Event& event){
...
//  BTagWeight_module->process(event);
  ...
  if(!sys_weights.empty()){
      for (unsigned i=2; i < 14; ++i) {
         cout << weights[i]/event.genInfo->originalXWGTUP() << endl;
      }
  }
  }
          cout << "---------" << endl;
...
UHH2_REGISTER_ANALYSIS_MODULE(ZprimeAnalysisModule_test)
