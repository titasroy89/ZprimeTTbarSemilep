...
for histName in histograms:
	...
	for sample in sample_names:
		...
		tree_MC[histName][sample]=_file[sample].Get("AnalysisTree")
        	tree_MC[histName][sample].Draw("%s>>h_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]))
		...
		legendR.AddEntry(hist[histName][sample],sample,'f')       
...
#stack.Draw('hist')
	...
	ratio.GetYaxis().SetRangeUser(min_,max_)
...
#minRatio = 0.5
	...
	if log:
		canvasRatio.SaveAs("%s_log.pdf"%(histName))
	else:
		canvasRatio.SaveAs("%s.pdf"%(histName))
