...
class ZprimePreselectionModule_forQCD : public ModuleBASE {

public:
  explicit ZprimePreselectionModule_forQCD(uhh2::Context&);
...
protected:
  ...
  std::unique_ptr<uhh2::Selection> triangular_inv_sel;
  ...
  void ZprimePreselectionModule_forQCD::init_JEC_JLC(uhh2::Context& ctx){
...
ZprimePreselectionModule_forQCD::ZprimePreselectionModule_forQCD(uhh2::Context& ctx){
...
//
  ...
  triangular_inv_sel.reset(new TriangularCut(0.02,1.5));
...
bool ZprimePreselectionModule_forQCD::process(uhh2::Event& event){
  ...
  if (isElectron){
  cout << "inverse triangular cut" <<endl;
  const bool pass_invtriang = triangular_inv_sel->passes(event);
  if(!pass_invtriang) return false;
  }
...
UHH2_REGISTER_ANALYSIS_MODULE(ZprimePreselectionModule_forQCD)
